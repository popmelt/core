...
function getSizingMode(value: string, authoredValue: string | null): SizingMode {
...
function getSizingValue(mode: SizingMode, currentValue: string): string {
  switch (mode) {
...
// Display mode types
type DisplayMode = 'block' | 'flex-row' | 'flex-col' | 'grid';

function getDisplayMode(display: string, flexDirection: string): DisplayMode {
...
function applyDisplayMode(mode: DisplayMode, handleChange: (property: string, value: string) => void) {
  switch (mode) {
    case 'block':
...
// Shared styles
const FIELD_BG = 'rgba(0, 0, 0, 0.04)';
...
function resolveUnitValue(input: string, property: string, currentValue: string, isModified: boolean): string {
...
function convertFromPx(px: number, targetUnit: string): number {
...
function FieldWrapper({ children, style, dimmed }: { children: React.ReactNode; modified?: boolean; style?: CSSProperties; dimmed?: boolean; accentColor?: string }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div
      onMouseEnter={() => setHovered(true)}
...
function InputWrapper({ children }: { children: React.ReactNode; modified?: boolean }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div
      onMouseEnter={() => setHovered(true)}
...
const BACKDROP_FILTERS: FilterDef[] = [
...
type ActiveFilter = { name: string; value: number };

function parseBackdropFilter(raw: string): ActiveFilter[] {
...
function serializeBackdropFilter(filters: ActiveFilter[]): string {
